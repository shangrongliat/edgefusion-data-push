...
func GenCertPrivateKey(dsa string, bits int) (*PrivateKey, error) {
	...
	default:
		return nil, errors.Errorf("unRecognized digital signature algorithm: %s", dsa)
	}
	return priv, nil
}
...
func EncodeCertPrivateKey(priv *PrivateKey) ([]byte, error) {
	...
	case EcPrivateKeyBlockType:
		...
		if err != nil {
			return nil, errors.Errorf("error mashaling ECPrivateKey: %s", err.Error())
		}
		block := pem.Block{
			Type:  EcPrivateKeyBlockType,
			Bytes: bytes,
		...
		return pem.EncodeToMemory(&block), nil
	default:
		return nil, errors.Errorf("unRecognized type of PrivateKey")
	}
}
