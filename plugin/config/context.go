...
func (c *Context) LoadBody(obj interface{}) error {
	...
	if err != nil {
		if es, ok := err.(validator.ValidationErrors); ok {
			for _, v := range es {
				return log.CustomError(common.Code(v.Tag()), log.CustomField(v.Tag(), v.Field()), log.CustomField("error", err.Error()))
			}
		}
		return err
	}
...
func PopulateFailedResponse(cc *Context, err error, abort bool) {
	...
	case log.Coder:
		code = e.Code()
		status = common.GetHTTPStatus(common.Code(e.Code()))
	default:
	...
	return func(c *gin.Context) {
		...
		defer func() {
			if r := recover(); r != nil {
				err, ok := r.(error)
				if !ok {
					err = log.CustomError(common.ErrUnknown, log.CustomField("error", r))
				}
...
func WrapperRaw(handler HandlerFunc, abort bool) func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if data, ok := res.([]byte); ok {
			...
			PopulateFailedResponse(cc, log.CustomError(common.ErrUnknown), abort)
		}
	}
}
